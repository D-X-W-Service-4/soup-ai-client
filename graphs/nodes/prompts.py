...
5. 출력은 반드시 문제 10개로 구성된 JSON 배열만 반환해야 합니다.
...
- 학생 풀이에 명시적 답(값/좌표/식/결론문장 등)이 있으면 "작성"으로 판정
- 아래 3줄 형식을 엄수해 작성한다:
...
[출력 규칙]
- <LOGIC> <FIT> <REQVAL> 세 태그 옆에만 짧게 한 줄로 작성한다.
- 나머지 정보, 장황한 해설은 금지한다.
...
[비교 절차]
1) 학생이 풀이 마지막에 적은 '최종 답'만 모두 추출한다.
2) '문제 정답'과 직접적으로 1:1 비교한다.
3) 최종 판정(정답 일치): [일치/일부 일치/불일치] 중 택일
...
[채점 규칙 — 매우 중요, 반드시 준수]
- 정답 비교 결과가 '일치'면 accuracy = 100, '불일치'면 accuracy = 0.
- 논리·적합성·작성결과 중 REQVAL이 '일부 누락/작성 안 함'이면 completeness = 0.
- FIT의 판단이 문제 의도와 다르면 relevance를 낮게 설정.
- step1에 '명백한 계산/대입 오류'가 있다면 validity도 낮게.
- presentation은 답변의 간결함·명확성에 따라 평가한다.

- 위 기준으로 accuracy, completeness, relevance, validity, presentation 점수를 산출한다.
- {max_score}점 만점에서 최종 score 산출.
- 최종적으로 대표적인 한 문장의 피드백을 feedback에 작성.
