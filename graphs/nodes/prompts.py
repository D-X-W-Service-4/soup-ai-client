...
오직 [문제]/[학생 풀이]/[학습 목표]의 텍스트만 근거로 판단하고, 새로운 계산이나 추정은 금지한다.
...
- 없으면 정확히 "논리 비약은 보이지 않음"이라고 써라.

2) <FIT> — 문제 의도·단원 적합성
- 풀이가 문제의 학습 목표와 맞는 접근인지 평가하라.
- 아래 3줄 형식을 그대로 써라.
  - 판단: (알맞음 / 문제 의도와 다름)
...
- 맞고/틀림은 여기서 판단하지 않는다(정오 판정은 추후에 할 예정)
- "명시적"의 예:
  - 변수=값 표기(예: x=2, y=3)
...
2) '문제 정답' 항목과 1:1로 비교한다.
3) 아래 판정을 만든다.
   - 최종 판정(정답 일치): [일치/일부 일치/불일치] 중 하나
